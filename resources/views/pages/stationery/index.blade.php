              ...
              <div class="card">
                  <div class="card-header">
                      ...
                      <div class="row  justify-content-between p-2" style="background-color: #F9F9FC;border-radius:10px;" >
                        <div class="col-4">
                          <form action="{{ url('/stationery') }}" class="d-flex" method="GET">
                            <input type="search" class="form-control" oninput="this.form.submit()" name="search" value="{{ request('search') }}" placeholder="Поиск"/>
                        </form>
                        </div>
                        <div class="col-8">
                          <a href="{{route('stationery.create')}}" class="btn btn-success float-end mx-2">
                  ...
                  <div class="card-body">
                      <div class="table-responsive mb-3 ">
                          <table class="table " id="roles-table">
                              ...
                              <tbody>
                                @forelse ($stationeries as $product)
                                <tr class="align-middle">
                                  <td>{{$product->name}}</td>
                                  <td>{{countStationery($product->id)}} {{$product->unit->name}}</td>
